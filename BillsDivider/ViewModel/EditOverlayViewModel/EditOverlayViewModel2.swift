...
final class EditOverlayViewModel2: ObservableObject {
    @Binding private var presenting: Bool

    @Published var priceText: String
    @Published var buyer: Buyer2
    @Published var owner: Owner2
    @Published var addAnother: Bool
    @Published var isPriceCorrect: Bool
    @Published var canConfirm: Bool

    @Published var buyers: [Buyer2]
    @Published var owners: [Owner2]

    let pricePlaceHolderText: String

    var getInitialBuyer: (() -> Buyer2)?
    var getInitialOwner: (() -> Owner2)?

    var positionAdded: AnyPublisher<ReceiptPosition2, Never>
    var positionEdited: AnyPublisher<ReceiptPosition2, Never>
    ...
    private let editOverlayStrategy: EditOverlayStrategy2
    private var subscriptions: [AnyCancellable]

    init(
        presenting: Binding<Bool>,
        editOverlayStrategy: EditOverlayStrategy2,
        ...
        self.positionAdded = Empty<ReceiptPosition2, Never>().eraseToAnyPublisher()
        self.positionEdited = Empty<ReceiptPosition2, Never>().eraseToAnyPublisher()
        self.subscriptions = []
    ...
    private func subscribe(to peopleDidUpdate: AnyPublisher<[Person], Never>) {
        peopleDidUpdate
            .sink { [weak self] in
                ...
                self.buyers = $0.map { Buyer2.person($0) }
                self.owners = $0.map { Owner2.person($0) } + [.all]
    ...
    private func tryCreateReceiptPosition() -> ReceiptPosition2? {
        guard let price = tryParsePrice(priceText) else {
            return nil
        }

        return ReceiptPosition2(amount: price, buyer: buyer, owner: owner)
    }
}
